      ...
      <div className="calculator-layout">
        <div className="calculator-main">
          <div className={styles.toolHeader}>
            <span className="category-badge"><span>⚖️</span> Seam Tool #93</span>
            ...
            {hasResult && (
              <div className={`calculator-results ${styles.results}`}>
                ...
                <div className={styles.resultDetails}>
                  {finishes.map(f=>(<div key={f.name} className={styles.resultRow} style={{flexDirection:"column",alignItems:"flex-start",gap:"0.25rem"}}><strong>{f.name}</strong><span style={{fontSize:"0.85rem"}}>⏱️ {f.time} · 🔧 {f.cost} · ✨ Best for: {f.best}</span></div>))}
                </div>
        ...
        <aside className="calculator-sidebar"><div className="glass-card related-tools"><h4>Related Tools</h4><a href="/seam-allowance/french-seam" className="related-tool-link">🇫🇷 French Seam</a><a href="/seam-allowance/flat-felled" className="related-tool-link">👖 Flat-Felled</a></div></aside>
      </div>
    </div>
  );
}