            ...
            <div className="calculator-layout">
                <div className="calculator-main">
                    ...
                    <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>
                        <button className={`btn ${triType === "turnover" ? "btn-primary" : "btn-secondary"}`} onClick={() => setTriType("turnover")}>
                            📐 Turnovers (Right △)
                        </button>
                        <button className={`btn ${triType === "equilateral" ? "btn-primary" : "btn-secondary"}`} onClick={() => setTriType("equilateral")}>
                            🔺 Equilateral △
                        </button>
                    </div>
                    ...
                    {triType === "turnover" && calc.type === "turnover" && (<>
                        <div className={`glass-card ${styles.calculatorCard}`}>
                            ...
                            <div style={{ fontSize: 11, marginTop: 4, color: "var(--color-text-secondary)" }}>
                                Total: <strong>{calc.totalTri} turnovers</strong> (from {Math.ceil(calc.totalTri / 2)} charm squares cut diagonally)
                            </div>
                        </div>

                        {/* BIAS WARNING */}
                        <div style={{ padding: 8, background: "hsl(40,30%,95%)", borderRadius: 6, fontSize: 11, marginBottom: 10, color: "hsl(40,70%,30%)", fontWeight: 600 }}>
                            ⚠️ Hypotenuse is on the BIAS — handle gently, pin before sewing, consider walking foot. Short legs are straight grain.
                        </div>
                    ...
                    {triType === "equilateral" && calc.type === "equilateral" && (<>
                        <div className={`glass-card ${styles.calculatorCard}`}>
                            ...
                            <div style={{ fontSize: 11, color: "var(--color-text-secondary)", marginTop: 4 }}>
                                Height: <strong>{calc.height.toFixed(2)}&quot;</strong> (side × 0.866) — All angles 60°
                            </div>
                        </div>
                        ...
                        <div className={`glass-card ${styles.calculatorCard}`} style={{ borderLeft: "4px solid hsl(120,50%,40%)" }}>
                            ...
                            <div style={{ fontSize: 11, color: "var(--color-text-tertiary)", marginTop: 6 }}>
                                Triangles alternate pointing up/down in rows. Top/bottom edges need partial triangles to square off.
                            </div>
                        </div>
                    </>)}
                    ...
                    {triType === "turnover" && (
                        <div className={`glass-card ${styles.calculatorCard}`}>
                            <button className="btn btn-sm btn-secondary" style={{ width: "100%", justifyContent: "space-between" }} onClick={() => setShowProjects(!showProjects)}>
                                📊 Pinwheel Quilt — Packs Needed
                    ...
                    {triType === "turnover" && calc.type === "turnover" && (
                        <div className={`glass-card ${styles.calculatorCard}`}>
                            <button className="btn btn-sm btn-secondary" style={{ width: "100%", justifyContent: "space-between" }} onClick={() => setShowBgCalc(!showBgCalc)}>
                                🧵 Background Fabric Calculator
                            ...
                            {showBgCalc && (
                                <div style={{ marginTop: 10, fontSize: 12, lineHeight: 2 }}>
                                    ...
                                    <div style={{ fontSize: 10, color: "var(--color-text-tertiary)", marginTop: 4 }}>
                                        Or: buy a second pack in light/background fabrics for coordinated variety.
                                    </div>
                                </div>
                            )}
                        </div>
                    ...
                    <div className={`glass-card ${styles.calculatorCard}`}>
                        <button className="btn btn-sm btn-secondary" style={{ width: "100%", justifyContent: "space-between" }} onClick={() => setShowEdu(!showEdu)}>
                            📚 Understanding Triangle Pre-Cuts
