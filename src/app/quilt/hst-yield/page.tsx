...
export default function Page() {
 ...
 <div style={{ fontSize: 11, color: "var(--color-text-tertiary)", marginTop: 4 }}>Unfinished: {toFrac(finSize + 0.5)}&quot; | Cut (2-at-a-time): {toFrac(calc.cut2)}&quot;
 </div>
 {/* Oversize */}
 ...
 <div style={{ fontSize: 11, color: "var(--color-text-tertiary)", marginTop: 4 }}>Fabric area: {calc.area} sq in ({roundN(calc.area / 144, 1)} sq ft)
 </div>
 </div>
 )}
 ...
 <div style={{ fontSize: 11, color: "var(--color-text-tertiary)", marginBottom: 8 }}>Add each fabric piece with its color. HSTs require matching light + dark pairs.
 </div>
 {pieces.map((p) =>{
 ...
 <div style={{ marginTop: 6, padding: 8, background: "hsl(150,20%,97%)", borderRadius: 6, fontSize: 12, fontWeight: 600 }}>Recommendation: Use {calc.enough4 ? "4-at-a-time" : calc.enough2 ? "2-at-a-time" : "8-at-a-time"} for best results with your fabric.
 </div>
 )}
 </div>
 )}
 ...
 <div style={{ fontSize: 10, color: "hsl(0,60%,50%)", marginTop: 2 }}>Limiting factor: {calc.limiting} fabric ({calc.limitingSq} pairs possible).
 {Math.abs(calc.lightSq - calc.darkSq)} unmatched {calc.limiting === "light" ? "dark" : "light"} squares.
 </div>
 )}
 </div>
 <div style={{ padding: 10, background: "hsl(280,15%,97%)", borderRadius: 6, fontSize: 14, fontWeight: 700, textAlign: "center" }}>Total: {calc.multiHst2} HSTs at {toFrac(finSize)}&quot; finished
 </div>
 <div style={{ fontSize: 11, marginTop: 4, color: "var(--color-text-tertiary)", textAlign: "center" }}>From {calc.limitingSq} matched pairs (2-at-a-time method)
 </div>
 </div>
 )}
 ...
 onClick={() =>setShowRef(!showRef)}>Fat Quarter Yield Reference (2-at-a-time)
 ...
 <div className={`glass-card ${styles.calculatorCard}`}>
 <button className="btn btn-sm btn-secondary" style={{ width: "100%", textAlign: "left", justifyContent: "space-between" }}
 onClick={() =>setShowYardRef(!showYardRef)}>Yield from 1 Yard (per color)
 ...
 <div className={`glass-card ${styles.calculatorCard}`}>
 <button className="btn btn-sm btn-secondary" style={{ width: "100%", textAlign: "left", justifyContent: "space-between" }}
 onClick={() =>setShowPrecut(!showPrecut)}>Pre-Cut Fabric HST Yield Guide
 ...
 <div className={`glass-card ${styles.calculatorCard}`}>
 <button className="btn btn-sm btn-secondary" style={{ width: "100%", textAlign: "left", justifyContent: "space-between" }}
 onClick={() =>setShowWhy(!showWhy)}>Why Yield Varies Between Methods
