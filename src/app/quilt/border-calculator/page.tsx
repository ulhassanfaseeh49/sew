...
export default function Page() {
 ...
 <input type="checkbox" checked={targetMode} onChange={e =>setTargetMode(e.target.checked)} style={{ accentColor: "var(--color-accent-primary)" }} />I want to reach a specific finished quilt size
 </label>
 {targetMode && (<>
 <div className="calculator-form-row">
 <div className="input-group">
 ...
 <AlertTriangle size={12} style={{ display: "inline", marginRight: 4 }} />Width and height need different border widths. Options:
 ...
 onClick={() =>setShowWidthOptions(!showWidthOptions)}>Border Width Options for Your Quilt <ChevronDown size={14} style={{ transform: showWidthOptions ? "rotate(180deg)" : "none", transition: "0.2s" }} />
 </button>
 {showWidthOptions && (
 <div style={{ marginTop: 10 }}>
 ...
 <div className={`glass-card ${styles.calculatorCard}`}>
 <button className="btn btn-sm btn-secondary" style={{ width: "100%", textAlign: "left", justifyContent: "space-between" }}
 onClick={() =>setShowEdu(!showEdu)}>Butted vs Mitered Corners — Complete Guide <ChevronDown size={14} style={{ transform: showEdu ? "rotate(180deg)" : "none", transition: "0.2s" }} />
 </button>
 {showEdu && (
 ...
 <p style={{ marginTop: 4 }}>Quilt edges stretch during handling — they are <strong>never accurate</strong>. Always measure your quilt through the center to determine border strip length. Cut borders to center measurement and ease to fit the edge. Borders cut to edge measurement result in wavy borders.
 </p>
 </div>
 )}
 </div>
