...
export default function Page() {
 ...
 ⚠ {blockSize}&quot; does not divide evenly by {grid}. Nearest valid: <strong>{nearLow}&quot;</strong>or <strong>{nearHigh}&quot;</strong>.
 ...
 {divisible && blockSize >0 && (
 <div style={{ fontSize: 11, color: "var(--color-text-tertiary)", marginTop: 4 }}>Chain square: {toFrac(blockSize / grid)}&quot; finished | Strip width: {toFrac(blockSize / grid + 0.5)}&quot;
 </div>
 )}
 </div>
 ...
 {(oddAcross || oddDown) && (
 <div style={{ marginTop: 6, padding: 8, background: "hsl(40,80%,95%)", borderRadius: 6, fontSize: 12, color: "hsl(30,70%,35%)" }}>
 ⚠ Irish Chain needs even block counts. {oddAcross && <span>Across: try {across - 1} or {across + 1}. </span>}{oddDown && <span>Down: try {down - 1} or {down + 1}.</span>}
 </div>
 )}
 {calc && (
 ...
 ⚠ <strong>Block B is NOT a plain square!</strong>It has chain-color squares in all 4 corners plus a large background center ({toFrac(calc.blockBCenterSize)}&quot; unfinished).
 </div>
 )}
 </div>
 )}
 ...
 <div style={{ marginTop: 6, fontSize: 12, fontWeight: 700, textAlign: "right" }}>Total: {(calc.buyChain + calc.buyBg).toFixed(2)} yards (blocks only)
 </div>
 ...
 onClick={() =>setShowRef(!showRef)}>Irish Chain Block Reference Tables
 ...
 <div className={`glass-card ${styles.calculatorCard}`}>
 <button className="btn btn-sm btn-secondary" style={{ width: "100%", justifyContent: "space-between" }}
 onClick={() =>setShowTrouble(!showTrouble)}>Troubleshooting Irish Chain
 ...
 <div className={`glass-card ${styles.calculatorCard}`}>
 <button className="btn btn-sm btn-secondary" style={{ width: "100%", justifyContent: "space-between" }}
 onClick={() =>setShowEdu(!showEdu)}>How Irish Chain Quilts Work
 ...
 <div style={{ fontSize: 12, color: "var(--color-text-secondary)", lineHeight: 1.9 }}>Mark an arrow on every Block B before assembly. Block B rotation is the #1 cause of broken chains in Double/Triple Irish Chain quilts.
 </div>
 </div>
