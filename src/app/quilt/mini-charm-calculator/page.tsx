...
export default function Page() {
 ...
 <input type="checkbox" checked={useIndividual} onChange={e =>setUseIndividual(e.target.checked)} />I have individual 2.5&quot; squares (not full packs)
 </label>
 </div>
 {useIndividual ? (
 ...
 <label style={{ fontSize: 11, display: "flex", alignItems: "center", gap: 6, cursor: "pointer" }}>
 <input type="checkbox" checked={hasJellyRoll} onChange={e =>setHasJellyRoll(e.target.checked)} />I also have a jelly roll from the same collection (+640 squares)
 </label>
 </div>
 </div>
 ...
 {/* ═══ MUG RUG CALCULATOR ═══ */}
 <div className={`glass-card ${styles.calculatorCard}`}>
 <button className="btn btn-sm btn-secondary" style={{ width: "100%", justifyContent: "space-between" }} onClick={() =>setShowMugRug(!showMugRug)}>Mug Rug Planner (Best Mini Charm Project!)
 ...
 {/* ═══ DOLL QUILT ═══ */}
 <div className={`glass-card ${styles.calculatorCard}`}>
 <button className="btn btn-sm btn-secondary" style={{ width: "100%", justifyContent: "space-between" }} onClick={() =>setShowDoll(!showDoll)}>Doll Quilt Planner
 ...
 {/* ═══ HSTs ═══ */}
 <div className={`glass-card ${styles.calculatorCard}`}>
 <button className="btn btn-sm btn-secondary" style={{ width: "100%", justifyContent: "space-between" }} onClick={() =>setShowHST(!showHST)}>HSTs from Mini Charms (Advanced)
 ...
 ⚠ <strong>HSTs from 2.5&quot; squares finish at only 1.5&quot; — very tiny!</strong>Requires precise ¼&quot; seam. Recommended for intermediate+ quilters.
 </div>
 ...
 {/* ═══ CORNERSTONES ═══ */}
 <div className={`glass-card ${styles.calculatorCard}`}>
 <button className="btn btn-sm btn-secondary" style={{ width: "100%", justifyContent: "space-between" }} onClick={() =>setShowCorner(!showCorner)}>Cornerstone Calculator
 <ChevronDown size={14} style={{ transform: showCorner ? "rotate(180deg)" : "none", transition: ".2s" }} />
 </button>
 {showCorner && (
 <div style={{ marginTop: 10, fontSize: 12, lineHeight: 1.9 }}>
 <div style={{ padding: 8, background: "hsl(0,0%,97%)", borderRadius: 6, marginBottom: 8, fontSize: 11 }}>Mini charms (2.5&quot; cut, 2&quot; finished) are the <strong>perfect size</strong>for cornerstones in quilts with 2&quot; sashing.
 </div>
 <div className="calculator-form-row">
 ...
 {/* ═══ JELLY ROLL COMBO ═══ */}
 <div className={`glass-card ${styles.calculatorCard}`}>
 <button className="btn btn-sm btn-secondary" style={{ width: "100%", justifyContent: "space-between" }} onClick={() =>setShowJelly(!showJelly)}>Jelly Roll + Mini Charm Combination
 ...
 ⚠ Full-Size Quilts from Mini Charms (Not Recommended)
 <ChevronDown size={14} style={{ transform: showFullSize ? "rotate(180deg)" : "none", transition: ".2s" }} />
 </button>
 {showFullSize && (
 <div style={{ marginTop: 10, fontSize: 12, lineHeight: 1.9 }}>
 <div style={{ padding: 8, background: "hsl(40,30%,96%)", borderRadius: 6, marginBottom: 8 }}>
 ⚠ Mini charm packs are NOT efficient for large quilts. Regular charm packs (5&quot; squares) require ¼ as many packs for the same area.
 </div>
 <table style={{ fontSize: 11, borderCollapse: "collapse", width: "100%" }}>
 ...
 {/* ═══ EDUCATIONAL ═══ */}
 <div className={`glass-card ${styles.calculatorCard}`}>
 <button className="btn btn-sm btn-secondary" style={{ width: "100%", justifyContent: "space-between" }} onClick={() =>setShowEdu(!showEdu)}>Understanding Mini Charms
