...
export default function Page() {
 ...
 {cw >0 && <div style={{ fontSize: 13, color: "var(--color-text-secondary)", marginTop: 8 }}>Finished binding width: ~{result ? result.finishedWidth.toFixed(2) : "—"}&quot;
 ...
 <div style={{ fontSize: 13, color: "var(--color-text-secondary)" }}>Pre-made binding is sold by length. The calculator will show how many yards/inches to purchase.
 </div>
 )}
 </div>
 ...
 <div style={{ fontSize: 13, color: "var(--color-text-secondary)" }}>Join strips with diagonal seams (45° angle)<br />Press seams open<br />
 {foldType === "double" ? "Fold strip in half lengthwise (wrong sides together)" : "Press under seam allowances on both long edges"}<br />Press well<br />Total joined length: ~{result.availableLen}&quot; ({(result.availableLen / 12).toFixed(1)} ft)<br />Your quilt needs: {result.totalLen}&quot; ({(result.totalLen / 12).toFixed(1)} ft)<br />Extra binding: ~{result.extraLen}&quot; ✓
 </div>
 </div>
 </div>
 )}
 ...
 <div style={{ fontSize: 12, color: "var(--color-text-tertiary)", marginTop: 8 }}>Common packages: 3 yd, 6 yd, 10 yd, 15 yd — choose the next size up that covers your needs.
 </div>
 </div>
 </div>
 )}
 ...
 <div className={`glass-card ${styles.calculatorCard}`}>
 <button className="btn btn-sm btn-secondary" style={{ width: "100%", textAlign: "left", justifyContent: "space-between" }}
 onClick={() =>setHaveMode(!haveMode)}>Do I Have Enough Binding Fabric? <ChevronDown size={14} style={{ transform: haveMode ? "rotate(180deg)" : "none", transition: "0.2s" }} />
 </button>
 {haveMode && (
 <div style={{ marginTop: 12 }}>
 <div className="calculator-form-row">
 ...
 <p style={{ fontSize: 12, color: "var(--color-text-tertiary)", marginBottom: 10 }}>Based on double-fold binding, 2.5&quot; cut strips, 42&quot; usable fabric width, standard allowances.
 </p>
 <div className={styles.tableWrap}>
 <table className={styles.convTable}>
 ...
 <div className={`glass-card ${styles.calculatorCard}`}>
 <button className="btn btn-sm btn-secondary" style={{ width: "100%", textAlign: "left", justifyContent: "space-between" }}
 onClick={() =>setShowEdu(!showEdu)}>Binding Basics &amp; Tips <ChevronDown size={14} style={{ transform: showEdu ? "rotate(180deg)" : "none", transition: "0.2s" }} />
 </button>
 {showEdu && (
