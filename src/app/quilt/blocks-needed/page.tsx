                ...
                <div className="calculator-main">
                    ...
                    <div className={`glass-card ${styles.calculatorCard}`}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                            <label style={{ fontSize: 13, display: "flex", alignItems: "center", gap: 6 }}>
                                <input type="checkbox" checked={isUnfinished} onChange={e => setIsUnfinished(e.target.checked)} style={{ accentColor: "var(--color-accent-primary)" }} />
                                Entering unfinished size
                            </label>
                        </div>
                    ...
                    <div className={`glass-card ${styles.calculatorCard}`}>
                        ...
                        <div style={{ display: "flex", gap: 6 }}>
                            <button className={`btn btn-sm ${layout === "straight" ? "btn-primary" : "btn-secondary"}`} onClick={() => setLayout("straight")}>
                                Straight Set (rows &amp; columns)
                            </button>
                            <button className={`btn btn-sm ${layout === "onpoint" ? "btn-primary" : "btn-secondary"}`} onClick={() => setLayout("onpoint")}>
                                On-Point / Diagonal
                            </button>
                        </div>
                    ...
                    <div className={`glass-card ${styles.calculatorCard}`}>
                        ...
                        <label style={{ fontSize: 13, display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                            <input type="checkbox" checked={hasSashing} onChange={e => setHasSashing(e.target.checked)} style={{ accentColor: "var(--color-accent-primary)" }} />
                            Include sashing between blocks
                        </label>
                        {hasSashing && (<>
                            <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 8 }}>
                                {SASH_PRESETS.map(s => (
                            ...
                            <label style={{ fontSize: 13, display: "flex", alignItems: "center", gap: 8 }}>
                                <input type="checkbox" checked={sashOuter} onChange={e => setSashOuter(e.target.checked)} style={{ accentColor: "var(--color-accent-primary)" }} />
                                Sashing on outer edges too
                            </label>
                        </>)}
                    </div>
                    ...
                    <div className={`glass-card ${styles.calculatorCard}`}>
                        ...
                        {totalBorder > 0 && (
                            <div style={{ fontSize: 12, color: "hsl(35,80%,45%)", marginTop: 4 }}>
                                ...
                                Borders use {toFrac(totalBorder)}&quot; total ({toFrac(totalBorder / 2)}&quot; each side), leaving {toFrac(availW)}&quot; × {toFrac(availH)}&quot; for blocks{hasSashing ? " + sashing" : ""}.
                            </div>
                        )}
                    </div>
                    ...
                    <div className={`glass-card ${styles.calculatorCard}`}>
                        ...
                        <label style={{ fontSize: 13, display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                            <input type="checkbox" checked={hasBlocks} onChange={e => setHasBlocks(e.target.checked)} style={{ accentColor: "var(--color-accent-primary)" }} />
                            I already have some blocks
                        </label>
                        {hasBlocks && (
                            <div className="input-group" style={{ maxWidth: 200 }}>
                            ...
                            {gapAnalysis && borders.length > 0 && (
                                <div style={{ background: "hsl(35,90%,95%)", borderRadius: "var(--radius-md)", padding: 14, marginTop: 10, fontSize: 13, lineHeight: 1.8 }}>
                                    ...
                                    <div style={{ marginTop: 6 }}>
                                        Your layout is {best.diffW >= 0 ? toFrac(best.diffW) + "\" wider" : toFrac(Math.abs(best.diffW)) + "\" narrower"} and {best.diffH >= 0 ? toFrac(best.diffH) + "\" taller" : toFrac(Math.abs(best.diffH)) + "\" shorter"} than target.
                    ...
                    <div className={`glass-card ${styles.calculatorCard}`}>
                        <button className="btn btn-sm btn-secondary" style={{ width: "100%", textAlign: "left", justifyContent: "space-between" }}
                            ...
                            📚 Why You Can&apos;t Always Hit an Exact Size <ChevronDown size={14} style={{ transform: showEdu ? "rotate(180deg)" : "none", transition: "0.2s" }} />
                        </button>
                        {showEdu && (
