...
export default function Page() {
 ...
 onClick={() => setMode(m.key)} style={{ textAlign: "left", padding: "12px 16px", height: "auto", display: "flex", flexDirection: "column", alignItems: "flex-start" }}>
 <div style={{ fontWeight: 600, fontSize: 13 }}>{m.label}</div>
 <div style={{ fontSize: 11, opacity: 0.7, marginTop: 3, lineHeight: 1.3 }}>{m.desc}</div>
 </button>
 ))}
 </div>
 ...
 <input type="checkbox" checked={isRect} onChange={e =>setIsRect(e.target.checked)} />Rectangular block (different width and height)
 </label>
 ...
 <Info size={12} style={{ display: "inline", marginRight: 4 }} />Seam allowance is added to each side: {toFrac(seamAllow)}&quot; × 2 sides = {toFrac(totalSA)}&quot; total per dimension.
 </div>
 </div>
 ...
 <AlertTriangle size={12} style={{ display: "inline", marginRight: 4 }} />Some pieces are very small — these can be tricky to handle. Consider a larger finished block size.
 </div>
 )}
 </div>
 )}
 ...
 <Info size={12} style={{ display: "inline", marginRight: 4 }} />A Nine-Patch has 2 seams across each direction. A Four-Patch has 1 seam.
 </div>
 ...
 onClick={() =>setShowPrecut(!showPrecut)}>Pre-Cut Fabric → Finished Block Size <ChevronDown size={14} style={{ transform: showPrecut ? "rotate(180deg)" : "none", transition: "0.2s" }} />
 </button>
 {showPrecut && (
 <div className={styles.tableWrap} style={{ marginTop: 12 }}>
 <table className={styles.convTable}>
 ...
 <div style={{ fontSize: 12, color: "var(--color-text-tertiary)", marginTop: 6 }}>Based on {toFrac(seamAllow)}&quot; seam allowance. One pre-cut = one plain block.
 </div>
 </div>
 )}
 </div>
 ...
 <div className={`glass-card ${styles.calculatorCard}`}>
 <button className="btn btn-sm btn-secondary" style={{ width: "100%", textAlign: "left", justifyContent: "space-between" }}
 onClick={() =>setShowRef(!showRef)}>Common Block Sizes Reference <ChevronDown size={14} style={{ transform: showRef ? "rotate(180deg)" : "none", transition: "0.2s" }} />
 </button>
 {showRef && (
 <div className={styles.tableWrap} style={{ marginTop: 12 }}>
 <table className={styles.convTable}>
 ...
 <div style={{ fontSize: 12, color: "var(--color-text-tertiary)", marginTop: 6 }}>Based on {toFrac(seamAllow)}&quot; seam allowance ({toFrac(totalSA)}&quot; total per dimension).
 </div>
 </div>
 )}
 </div>
 ...
 <div className={`glass-card ${styles.calculatorCard}`}>
 <button className="btn btn-sm btn-secondary" style={{ width: "100%", textAlign: "left", justifyContent: "space-between" }}
 onClick={() =>setShowEdu(!showEdu)}>Understanding Block Sizes <ChevronDown size={14} style={{ transform: showEdu ? "rotate(180deg)" : "none", transition: "0.2s" }} />
 </button>
 {showEdu && (
