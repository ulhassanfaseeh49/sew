...
export default function Page() {
 ...
 {/* ═══ WIDTH COMPARISON ═══ */}
 <div className={`glass-card ${styles.calculatorCard}`}>
 <button className="btn btn-sm btn-secondary" style={{ width: "100%", justifyContent: "space-between" }} onClick={() =>setShowCompare(!showCompare)}>Sashing Width Comparison
 ...
 {/* ═══ REVERSE CALC ═══ */}
 <div className={`glass-card ${styles.calculatorCard}`}>
 <button className="btn btn-sm btn-secondary" style={{ width: "100%", justifyContent: "space-between" }} onClick={() =>setShowReverse(!showReverse)}>Target Size → Sashing Width
 ...
 ⚠ Width and height need different sashing widths. Consider using {toF(Math.max(reverse.nearestW, reverse.nearestH))} throughout and adjusting with borders.
 </div>
 )}
 <div style={{ marginTop: 6, padding: 6, background: "hsl(160,15%,96%)", borderRadius: 4, fontSize: 11 }}>With {toF(reverse.nearestW)} sashing (width-matched): {reverse.actualW}&quot; × {(reverse.blocksOnlyH + reverse.sr * reverse.nearestW).toFixed(1)}&quot;
 </div>
 </div>
 )}
 </div>
 ...
 {/* ═══ SCRAPPY ═══ */}
 <div className={`glass-card ${styles.calculatorCard}`}>
 <button className="btn btn-sm btn-secondary" style={{ width: "100%", justifyContent: "space-between" }} onClick={() =>setShowScrappy(!showScrappy)}>Scrappy Sashing Planner
 ...
 {/* ═══ PIANO KEY ═══ */}
 <div className={`glass-card ${styles.calculatorCard}`}>
 <button className="btn btn-sm btn-secondary" style={{ width: "100%", justifyContent: "space-between" }} onClick={() =>setShowPiano(!showPiano)}>Piano Key Sashing
 ...
 {Math.abs(actualLen - blockSize) >0.01 && <div className="result-row" style={{ color: "hsl(40,70%,40%)" }}><span>⚠ Keys don&apos;t divide evenly</span><strong>Actual: {toF(actualLen)} (adjust key size)</strong></div>}
 ...
 <div style={{ marginTop: 6, fontSize: 11, color: "var(--color-text-tertiary)" }}>Assembly: Sew {keysPerStrip} keys end-to-end per strip → press seams one direction → resulting strip: {toF(cutW)} × {toF(blockSize + SA)}
 </div>
 </>
 );
 })()}
 ...
 {/* ═══ EDUCATIONAL ═══ */}
 <div className={`glass-card ${styles.calculatorCard}`}>
 <button className="btn btn-sm btn-secondary" style={{ width: "100%", justifyContent: "space-between" }} onClick={() =>setShowEdu(!showEdu)}>Understanding Sashing
 ...
 <div style={{ fontSize: 11, padding: 8, background: "hsl(0,0%,96%)", borderRadius: 6, fontFamily: "monospace", lineHeight: 2 }}>Between only:<br />CS = (cols−1)×(rows−1)<br />Full sashing:<br />CS = (cols+1)×(rows+1)<br />Cut: sashW + ½&quot;
 </div>
 </div>
