...
export default function Page() {
 ...
 <div style={{ fontSize: 12, color: "var(--color-text-tertiary)", marginTop: 4 }}>Total area: {totalArea.toLocaleString()} sq inches
 </div>
 </div>
 ...
 <div style={{ fontSize: 11, color: "var(--color-text-tertiary)", marginTop: 6, textAlign: "center" }}>Simulated 12×10 block grid showing approximate color distribution. Click Shuffle for new arrangement.
 </div>
 </div>
 )}
 ...
 <div className={styles.resultRow}><span>Light values</span><strong>{analysis.lightPct}%</strong></div>
 ...
 <div style={{ fontSize: 12, color: "var(--color-text-tertiary)", marginBottom: 8 }}>Based on {width}&quot; × {height}&quot; quilt ({totalArea.toLocaleString()} sq in) at {fabricW}&quot; fabric width
 </div>
 ...
 <div style={{ fontSize: 12, color: "var(--color-text-tertiary)", marginTop: 6 }}>Includes 10% waste buffer. For directional prints, add 15-20% extra. Yardage formula: (Quilt Area × Proportion) ÷ (Usable Width × 36).
 </div>
 </div>
 )}
 ...
 onClick={() =>setShowEdu(!showEdu)}>The 60-30-10 Rule — A Guide for Quilters <ChevronDown size={14} style={{ transform: showEdu ? "rotate(180deg)" : "none", transition: "0.2s" }} />
 </button>
 {showEdu && (
