...
export default function Page() {
 ...
 <div style={{ marginTop: 6, fontSize: 11, color: "var(--color-text-tertiary)" }}>Angle per wedge: {c.angle}° | Half angle: {c.halfAngle}°
 </div>
 </div>
 ...
 <div style={{ fontSize: 11, color: "var(--color-text-tertiary)", marginTop: 4 }}>Block size: {toFrac(c.blockSize)}&quot; finished ({toFrac(c.cutBlock)}&quot; cut) | Radius: {toFrac(c.radius)}&quot;
 </div>
 </div>
 ...
 <div style={{ marginTop: 6, fontSize: 12, fontWeight: 700, textAlign: "right" }}>Total: {(c.buyKaleido + c.buyBg + c.buyCenter).toFixed(2)} yards
 </div>

 {hasRepeat && c.wastePercent >0 && (
 <div style={{ marginTop: 8, padding: 10, background: "hsl(0,25%,97%)", borderRadius: 6, fontSize: 11 }}>
 <strong>⚠ Why so much kaleidoscope fabric?</strong><br />Repeat matching requires cutting from the same position in each {repeatLen}&quot; repeat.
 ...
 <div style={{ marginTop: 4 }}>Without repeat: ~{c.naiveYd.toFixed(1)} yd | With repeat: ~{c.kaleidoYd.toFixed(1)} yd | Extra: +{(c.kaleidoYd - c.naiveYd).toFixed(1)} yd
 </div>
 )}
 </div>
 )}
 ...
 onClick={() =>setShowAngleRef(!showAngleRef)}>Wedge Angle Quick Reference
 ...
 <div className={`glass-card ${styles.calculatorCard}`}>
 <button className="btn btn-sm btn-secondary" style={{ width: "100%", justifyContent: "space-between" }}
 onClick={() =>setShowFabricGuide(!showFabricGuide)}>Fabric Suitability Guide
 ...
 <div className={`glass-card ${styles.calculatorCard}`}>
 <button className="btn btn-sm btn-secondary" style={{ width: "100%", justifyContent: "space-between" }}
 onClick={() =>setShowEdu(!showEdu)}>How Kaleidoscope Quilts Work
 ...
 <div style={{ fontSize: 12, color: "var(--color-text-secondary)", lineHeight: 1.9 }}>Every wedge must be cut from the exact same x,y position within the fabric repeat. If even one wedge is off, the kaleidoscope symmetry breaks.
 </div>
 </div>
