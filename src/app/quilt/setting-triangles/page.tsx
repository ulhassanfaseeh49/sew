                ...
                <div className="calculator-main">
                    ...
                    <div className={`glass-card ${styles.calculatorCard}`}>
                        ...
                        <label style={{ fontSize: 12, display: "flex", alignItems: "center", gap: 6, cursor: "pointer", marginBottom: 6 }}>
                            ...
                            Cut oversized for trimming (recommended)
                        </label>
                        {oversize && (
                    ...
                    <div className={`glass-card ${styles.calculatorCard}`} style={{ borderLeft: "4px solid hsl(150,50%,40%)" }}>
                        ...
                        <div style={{ marginTop: 8, fontSize: 11, color: "var(--color-text-tertiary)" }}>
                            Approx on-point quilt size: {calc.quiltW.toFixed(0)}&quot; × {calc.quiltH.toFixed(0)}&quot; (before borders)
                        </div>
                    </div>
                    ...
                    <div className={`glass-card ${styles.calculatorCard}`}>
                        <button className="btn btn-sm btn-secondary" style={{ width: "100%", justifyContent: "space-between" }} onClick={() => setShowFormula(!showFormula)}>
                            📐 How These Sizes Were Calculated
                        ...
                        {showFormula && (
                            <div style={{ marginTop: 10, fontSize: 12, lineHeight: 2 }}>
                                <div style={{ padding: 10, background: "hsl(150,10%,97%)", borderRadius: 6, marginBottom: 8 }}>
                                    ...
                                    <div style={{ fontFamily: "monospace", fontSize: 11, marginTop: 4, lineHeight: 1.8 }}>
                                        Block diagonal = {blockSize}&quot; × √2 = {calc.diag.toFixed(3)}&quot;<br />
                                        Square = diagonal + 1.25&quot; = {calc.diag.toFixed(3)} + 1.25 = {calc.sideRaw.toFixed(3)}&quot;<br />
                                        Round up to ⅛&quot;: {toF(roundUp8(calc.sideRaw))}{oversize ? ` + ${toF(oversizeAmt)} oversize = ${toF(calc.sideSq)}` : ""}
                                    </div>
                                    <div style={{ fontSize: 11, color: "var(--color-text-tertiary)", marginTop: 4 }}>
                                        Cut TWICE diagonally → 4 quarter-square triangles.<br />
                                        Straight grain on long edge (quilt&apos;s outside edge) ✓
                                    </div>
                                </div>
                                <div style={{ padding: 10, background: "hsl(30,10%,97%)", borderRadius: 6 }}>
                                    ...
                                    <div style={{ fontFamily: "monospace", fontSize: 11, marginTop: 4, lineHeight: 1.8 }}>
                                        Block diagonal / 2 = {calc.diag.toFixed(3)} / 2 = {(calc.diag / 2).toFixed(3)}&quot;<br />
                                        Square = half-diag + 0.875&quot; = {(calc.diag / 2).toFixed(3)} + 0.875 = {calc.cornerRaw.toFixed(3)}&quot;<br />
                                        Round up to ⅛&quot;: {toF(roundUp8(calc.cornerRaw))}{oversize ? ` + ${toF(oversizeAmt)} oversize = ${toF(calc.cornerSq)}` : ""}
                                    </div>
                                    <div style={{ fontSize: 11, color: "var(--color-text-tertiary)", marginTop: 4 }}>
                                        Cut ONCE diagonally → 2 half-square triangles.<br />
                                        Straight grain on two short legs (quilt&apos;s corner edges) ✓
                                    </div>
                                </div>
                            </div>
                        )}
                    ...
                    <div className={`glass-card ${styles.calculatorCard}`}>
                        <button className="btn btn-sm btn-secondary" style={{ width: "100%", justifyContent: "space-between" }} onClick={() => setShowCompare(!showCompare)}>
                            📊 Standard vs Oversized Comparison
                    ...
                    <div className={`glass-card ${styles.calculatorCard}`}>
                        <button className="btn btn-sm btn-secondary" style={{ width: "100%", justifyContent: "space-between" }} onClick={() => setShowEdu(!showEdu)}>
                            📚 Understanding Setting Triangles
                ...
                <aside className="calculator-sidebar">
                    ...
                    <div className="glass-card" style={{ padding: "var(--space-5)", marginBottom: "var(--space-4)" }}>
                        ...
                        <div style={{ fontSize: 11, padding: 8, background: "hsl(0,0%,96%)", borderRadius: 6, fontFamily: "monospace", lineHeight: 2 }}>
                            Side = block × √2 + 1.25&quot;<br />
                            Corner = block × √2 ÷ 2 + 0.875&quot;<br />
                            Side: cut TWICE → 4 tri<br />
                            Corner: cut ONCE → 2 tri
                        </div>
                    </div>
