...
export default function Page() {
 ...
 <strong>{bType.label}</strong>— {bType.fiber}<br />Loft: {bType.loft} · Quilting distance: {bType.qDist} · Warmth: {bType.warmth}<br />Best for: {bType.best}<br />Price: {bType.price} · Shrinkage: {bType.shrink >0 ? `~${bType.shrink}%` : "None"}
 </div>
 <label style={{ display: "flex", gap: 6, alignItems: "center", fontSize: 13, marginTop: 10, color: "var(--color-text-secondary)" }}>
 <input type="checkbox" checked={preWash} onChange={e =>setPreWash(e.target.checked)} />Pre-wash batting? {preWash && bType.shrink >0 && <span style={{ color: "var(--color-text-tertiary)" }}>(adds {bType.shrink}% to dimensions)</span>}
 </label>
 </div>
 )}
 </div>
 ...
 <strong>⚠ Baby Quilt Safety:</strong>Use <strong>low-loft</strong>batting only. High loft is not recommended for baby sleep quilts. Organic cotton or bamboo are ideal for sensitive skin.
 </div>
 )}
 ...
 <div className="result-label">Required batting size · {(result.bw * 2.54).toFixed(0)} × {(result.bh * 2.54).toFixed(0)} cm
 </div>
 </div>
 ...
 <div style={{ fontSize: 13, color: "var(--color-text-secondary)" }}>Gives you {result.recommended.sparW}&quot; extra width and {result.recommended.sparH}&quot; extra height.
 </div>
 </div>
 )}
 ...
 <p style={{ fontSize: 12, color: "var(--color-text-tertiary)", marginBottom: 10 }}>Need {result.bh}&quot; of length. Standard batting rolls come in these widths:
 </p>
 <div className={styles.tableWrap}>
 <table className={styles.convTable}>
 ...
 <div className={`glass-card ${styles.calculatorCard}`}>
 <button className="btn btn-sm btn-secondary" style={{ width: "100%", textAlign: "left", justifyContent: "space-between" }}
 onClick={() =>setHaveMode(!haveMode)}>Do I Have Enough Batting? <ChevronDown size={14} style={{ transform: haveMode ? "rotate(180deg)" : "none", transition: "0.2s" }} />
 </button>
 {haveMode && (
 <div style={{ marginTop: 12 }}>
 <div className="calculator-form-row">
 ...
 <div className={`glass-card ${styles.calculatorCard}`}>
 <button className="btn btn-sm btn-secondary" style={{ width: "100%", textAlign: "left", justifyContent: "space-between" }}
 onClick={() =>setShowEdu(!showEdu)}>Batting Basics for Beginners <ChevronDown size={14} style={{ transform: showEdu ? "rotate(180deg)" : "none", transition: "0.2s" }} />
 </button>
 {showEdu && (
 ...
 <p style={{ fontSize: 12, color: "var(--color-text-tertiary)" }}>Always check the manufacturer&apos;s specific requirements — they vary by product even within the same fiber type.
 </p>
 </div>
 </div>
