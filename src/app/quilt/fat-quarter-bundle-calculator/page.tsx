                ...
                <div className="calculator-main">
                    ...
                    <div className={`glass-card ${styles.calculatorCard}`}>
                        ...
                        <div style={{ marginTop: 8, padding: 10, background: "hsl(200,25%,96%)", borderRadius: 6, fontSize: 12, lineHeight: 1.7, color: "var(--color-text-secondary)" }}>
                            <strong>Fat Quarter:</strong> 18&quot; × 22&quot; (usable: {toFrac(FQ_W)}&quot; × {FQ_H}&quot;) • Area: 396 sq&quot; each<br />
                            Total fabric: <strong>{fqCount} × 396 = {(fqCount * 396).toLocaleString()} sq&quot;</strong> ({toFrac(fqCount * 0.25)} yards equivalent)
                        </div>
                    </div>
                    ...
                    <div className={`glass-card ${styles.calculatorCard}`}>
                        ...
                        {results.tooLarge && (
                            <div style={{ marginTop: 8, padding: 8, background: "hsl(0,40%,95%)", borderRadius: 6, fontSize: 12, color: "hsl(0,60%,45%)", fontWeight: 600 }}>
                                ⚠️ A {blockSize}&quot; finished block requires {toFrac(results.cutSize)}&quot; cuts — too large for a fat quarter ({toFrac(FQ_W)}&quot; × {FQ_H}&quot;). Maximum block size: 17&quot;.
                            </div>
                        )}
                    </div>
                    ...
                    <div className={`glass-card ${styles.calculatorCard}`}>
                        ...
                        <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", fontSize: 13 }}>
                            <input type="checkbox" checked={useBackground} onChange={e => setUseBackground(e.target.checked)}
                                style={{ width: 18, height: 18, accentColor: "hsl(150,60%,40%)" }} />
                            Add background alternate blocks to double quilt size
                        </label>
                        {useBackground && (
                        ...
                        <div className={`glass-card ${styles.calculatorCard}`} style={{ borderLeft: "4px solid hsl(150,60%,40%)" }}>
                            ...
                            {useBackground && results.bgLayout && (
                                <div style={{ padding: 12, background: "hsl(200,25%,96%)", borderRadius: 6, marginTop: 8 }}>
                                    <div style={{ fontSize: 13, fontWeight: 700, color: "hsl(200,60%,35%)", marginBottom: 4 }}>
                                        🔲 With Background Alternates
                                    </div>
                                    <div style={{ fontSize: 14, fontWeight: 600 }}>
                            ...
                            <div style={{ marginTop: 10, padding: 10, background: "hsl(45,30%,96%)", borderRadius: 6 }}>
                                <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 4, color: "hsl(45,50%,35%)" }}>♻️ Remnants per FQ After Cutting</div>
                                <div style={{ fontSize: 12, lineHeight: 1.7, color: "var(--color-text-secondary)" }}>
                                    ...
                                    <div style={{ fontSize: 11, marginTop: 3, color: "var(--color-text-tertiary)" }}>
                                        Total from {fqCount} FQs: {results.remnantStripA.w > 0.5 ? `${fqCount} side strips` : ""}{results.remnantStripA.w > 0.5 && results.remnantStripB.h > 0.5 ? " + " : ""}{results.remnantStripB.h > 0.5 ? `${fqCount} bottom strips` : ""} — save for a bonus project!
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className={`glass-card ${styles.calculatorCard}`}>
                        <button className="btn btn-sm btn-secondary" style={{ width: "100%", textAlign: "left", justifyContent: "space-between" }}
                            ...
                            📐 Blocks per Fat Quarter Reference
                                    ...
                                    <tbody>
                                        {blockRefRows.map(r => (
                                            <tr key={r.size} style={{ background: r.size === blockSize ? "hsl(150,40%,95%)" : undefined, borderBottom: "1px solid hsl(0,0%,92%)" }}>
                                                ...
                                                <td style={{ textAlign: "right", padding: "5px 6px", fontWeight: 600 }}>{r.tooLarge ? "⚠️" : r.per}</td>
                    ...
                    <div className={`glass-card ${styles.calculatorCard}`}>
                        <button className="btn btn-sm btn-secondary" style={{ width: "100%", textAlign: "left", justifyContent: "space-between" }}
                            ...
                            ⚖️ Fat Quarter vs Regular Quarter Yard
                        ...
                        {showComparison && (
                            <div style={{ marginTop: 10 }}>
                                ...
                                <div style={{ marginTop: 8, fontSize: 12, fontWeight: 600, color: "hsl(150,60%,35%)", textAlign: "center" }}>
                                    Same area — completely different utility. Fat quarters are vastly more useful for blocks!
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    <div className={`glass-card ${styles.calculatorCard}`}>
                        <button className="btn btn-sm btn-secondary" style={{ width: "100%", textAlign: "left", justifyContent: "space-between" }}
                            onClick={() => setShowBundleRef(!showBundleRef)}>
                            📦 Bundle Size Project Guide
