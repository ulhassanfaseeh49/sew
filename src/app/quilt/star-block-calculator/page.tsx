                ...
                <div className="calculator-main">
                    ...
                    {star.yseam && (
                        <div style={{ padding: 8, background: "hsl(40,30%,95%)", borderRadius: 6, fontSize: 11, marginBottom: 10, color: "hsl(40,70%,30%)", fontWeight: 600 }}>
                            ⚠️ {star.name} requires Y-seams (set-in seams) — intermediate technique. Consider Ohio Star or Sawtooth Star for a no-Y-seam alternative.
                        </div>
                    )}
                    ...
                    <div className={`glass-card ${styles.calculatorCard}`} style={{ borderLeft: `4px solid hsl(${starType === "ohio" ? "200" : starType === "sawtooth" ? "340" : starType === "variable" ? "120" : "40"},50%,40%)` }}>
                        ...
                        <div style={{ marginTop: 6, fontSize: 11, textAlign: "center", color: "var(--color-text-tertiary)" }}>
                            Total: ~{calc.totalYd.toFixed(1)} yd across all fabrics for {blockCount} blocks
                        </div>
                    </div>
                            ...
                            <div style={{ marginTop: 10, fontSize: 12, lineHeight: 2, fontFamily: "monospace" }}>
                                {starType === "ohio" && (
                                    <div style={{ padding: 10, background: "hsl(200,10%,97%)", borderRadius: 6 }}>
                                        Block = 3×3 grid → section = {blockSize} ÷ 3 = {(blockSize / 3).toFixed(2)}&quot;<br />
                                        Center / Corner cut = section + ½&quot; SA = {toF(blockSize / 3 + SA)}<br />
                                        QST starting square = section + 1¼&quot; = {toF(blockSize / 3 + QST_ADD)}<br />
                                        Per block: 1 center + 4 corners + 2 dark QST + 2 bg QST = 9 pieces
                                    </div>
                                )}
                                {starType === "sawtooth" && (
                                    <div style={{ padding: 10, background: "hsl(340,10%,97%)", borderRadius: 6 }}>
                                        Center = block ÷ 2 + ½&quot; = {toF(blockSize / 2 + SA)}<br />
                                        Unit = block ÷ 4 = {(blockSize / 4).toFixed(2)}&quot;<br />
                                        Geese rectangle = {toF(blockSize / 2 + SA)} × {toF(blockSize / 4 + SA)}<br />
                                        Sky / Corner squares = unit + ½&quot; = {toF(blockSize / 4 + SA)}<br />
                                        Per block: 1 center + 4 geese rect + 8 sky sq + 4 corners = 17 pieces
                                    </div>
                                )}
                                {starType === "variable" && (
                                    <div style={{ padding: 10, background: "hsl(120,10%,97%)", borderRadius: 6 }}>
                                        Block = 3×3 grid → section = {blockSize} ÷ 3 = {(blockSize / 3).toFixed(2)}&quot;<br />
                                        Center / Corner cut = section + ½&quot; = {toF(blockSize / 3 + SA)}<br />
                                        HST starting square = section + ⅞&quot; = {toF(blockSize / 3 + 0.875)}<br />
                                        Per block: 1 center + 4 corners + 4 dark HST + 4 bg HST = 13 pieces
                                    </div>
                                )}
                                {starType === "lemoyne" && (
                                    <div style={{ padding: 10, background: "hsl(40,10%,97%)", borderRadius: 6 }}>
                                        Diamond strip width ≈ block ÷ 3 + ½&quot; = {toF(blockSize / 3 + SA)}<br />
                                        Cut 45° diamonds from strips<br />
                                        Setting squares = {toF(blockSize / 3 + SA)}<br />
                                        Setting triangle squares = {toF(blockSize / 3 + 0.875)} (cut once diag)<br />
                                        Per block: 8 diamonds + 4 setting sq + 4 setting tri = 16 pieces
                                    </div>
                                )}
                            </div>
                        )}
