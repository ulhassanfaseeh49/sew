...
export default function Page() {
 ...
 ⚠ <strong>Safety:</strong>The AAP recommends NO loose bedding in cribs for babies under 12 months. Baby quilts are safest as play mats, stroller covers, and decorative items.
 </div>
 ...
 <div style={{ fontSize: 11, color: "var(--color-text-secondary)", marginTop: 6 }}>Layout: {calc.blocksAcross} × {calc.blocksDown} = <strong>{calc.totalBlocks} blocks</strong>at {blockSize}&quot;{hasBorder ? ` + ${borderW}" border` : ""}
 </div>
 </div>
 ...
 {/* REFERENCE TABLE */}
 <div className={`glass-card ${styles.calculatorCard}`}>
 <button className="btn btn-sm btn-secondary" style={{ width: "100%", justifyContent: "space-between" }} onClick={() =>setShowRef(!showRef)}>Baby Quilt Quick Reference
 ...
 <div>Batting: <strong>{calc.batPkg}</strong></div>
 </div>
 </div>
