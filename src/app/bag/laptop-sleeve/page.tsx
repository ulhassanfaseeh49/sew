...
const sizes = [{"k":"13","n":"13\" Laptop","w":13,"h":9},{"k":"14","n":"14\" Laptop","w":14,"h":10},{"k":"15","n":"15\" Laptop","w":15,"h":10.5},{"k":"16","n":"16\" Laptop","w":16,"h":11}];
export default function Page() {
    const [sizeKey,setSizeKey]=useState(sizes[1]?.k||sizes[0].k);
    const [qty,setQty]=useState(1);
    const [activeFaq,setActiveFaq]=useState<number|null>(null);
    const faqItems=[{"q":"What padding for laptop sleeves?","a":"Foam interfacing (Soft and Stable by ByAnnie): best protection, 1/4 inch thick. Fusible fleece: lighter padding. Batting: softest but least protective."}];
    ...
    return (<div className="container"><Breadcrumb items={[{label:"Bags",href:"/bag"},{label:"Laptop Sleeve"}]}/>
        <div className="calculator-layout"><div className="calculator-main">
        <div className={styles.toolHeader}><span className="category-badge"><ShoppingBag size={14}/> Bag #380</span><h1>Laptop Sleeve Calculator</h1><p>Padded fabric for laptop sleeves.</p></div>
        ...
        <div className="toolbar" style={{marginBottom:10}}>
            <button className="btn btn-secondary btn-sm" onClick={()=>navigator.clipboard.writeText(`${qty} Laptop: ${toFrac(calc.yd)} yd`)}><ClipboardCopy size={13}/> Copy</button>
