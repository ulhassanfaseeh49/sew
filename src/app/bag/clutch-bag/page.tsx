...
const sizes = [{"k":"small","n":"Card Clutch (7×4)","w":7,"h":4},{"k":"std","n":"Standard (10×6)","w":10,"h":6},{"k":"large","n":"Oversized (12×7)","w":12,"h":7},{"k":"fold","n":"Foldover (10×12→6)","w":10,"h":12}];
export default function Page() {
    const [sizeKey,setSizeKey]=useState(sizes[1]?.k||sizes[0].k);
    const [qty,setQty]=useState(1);
    const [activeFaq,setActiveFaq]=useState<number|null>(null);
    const faqItems=[{"q":"Best fabric for clutch bags?","a":"Silk, satin, brocade for evening. Cork or faux leather for casual. Always use heavyweight interfacing for structure."}];
    ...
    return (<div className="container"><Breadcrumb items={[{label:"Bags",href:"/bag"},{label:"Clutch/Evening Bag"}]}/>
        <div className="calculator-layout"><div className="calculator-main">
        <div className={styles.toolHeader}><span className="category-badge"><ShoppingBag size={14}/> Bag #378</span><h1>Clutch/Evening Bag Calculator</h1><p>Fabric for clutch bags.</p></div>
        ...
        <div className="toolbar" style={{marginBottom:10}}>
            <button className="btn btn-secondary btn-sm" onClick={()=>navigator.clipboard.writeText(`${qty} Clutch/Evening: ${toFrac(calc.yd)} yd`)}><ClipboardCopy size={13}/> Copy</button>
