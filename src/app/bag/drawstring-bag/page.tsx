...
const sizes = [{"k":"small","n":"Small (6×8)","w":6,"h":8},{"k":"med","n":"Medium (10×12)","w":10,"h":12},{"k":"large","n":"Large (14×16)","w":14,"h":16},{"k":"laundry","n":"Laundry (18×24)","w":18,"h":24}];
export default function Page() {
    const [sizeKey,setSizeKey]=useState(sizes[1]?.k||sizes[0].k);
    const [qty,setQty]=useState(1);
    const [activeFaq,setActiveFaq]=useState<number|null>(null);
    const faqItems=[{"q":"How much cord for a drawstring bag?","a":"Cord length = bag width × 2 + 12 inches (for tying). Two cords for a double-drawstring: 2x that amount."}];
    ...
    return (<div className="container"><Breadcrumb items={[{label:"Bags",href:"/bag"},{label:"Drawstring Bag"}]}/>
        <div className="calculator-layout"><div className="calculator-main">
        <div className={styles.toolHeader}><span className="category-badge"><ShoppingBag size={14}/> Bag #375</span><h1>Drawstring Bag Calculator</h1><p>Calculate fabric for drawstring bags.</p></div>
        ...
        <div className="toolbar" style={{marginBottom:10}}>
            <button className="btn btn-secondary btn-sm" onClick={()=>navigator.clipboard.writeText(`${qty} Drawstring: ${toFrac(calc.yd)} yd`)}><ClipboardCopy size={13}/> Copy</button>
