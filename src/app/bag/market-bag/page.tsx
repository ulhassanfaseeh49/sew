...
const sizes = [{"k":"small","n":"Small (10×12×4)","w":10,"h":12,"d":4},{"k":"std","n":"Standard (13×15×5)","w":13,"h":15,"d":5},{"k":"large","n":"Large (15×17×6)","w":15,"h":17,"d":6}];
export default function Page() {
    const [sizeKey,setSizeKey]=useState(sizes[1]?.k||sizes[0].k);
    const [qty,setQty]=useState(1);
    const [activeFaq,setActiveFaq]=useState<number|null>(null);
    const faqItems=[{"q":"Best fabric for reusable bags?","a":"Ripstop nylon (lightest, folds tiny). Canvas: sturdy, washable. Mesh: great for produce. Cotton: eco-friendly, washable."}];
    ...
    return (<div className="container"><Breadcrumb items={[{label:"Bags",href:"/bag"},{label:"Market/Grocery Bag"}]}/>
        <div className="calculator-layout"><div className="calculator-main">
        <div className={styles.toolHeader}><span className="category-badge"><ShoppingBag size={14}/> Bag #376</span><h1>Market/Grocery Bag Calculator</h1><p>Calculate fabric for reusable market bags.</p></div>
        ...
        <div className="toolbar" style={{marginBottom:10}}>
            <button className="btn btn-secondary btn-sm" onClick={()=>navigator.clipboard.writeText(`${qty} Market/Grocery: ${toFrac(calc.yd)} yd`)}><ClipboardCopy size={13}/> Copy</button>
