...
const sizes = [{"k":"small","n":"Daypack (10×14×5)","w":10,"h":14,"d":5},{"k":"med","n":"Standard (12×16×6)","w":12,"h":16,"d":6},{"k":"large","n":"Full (14×18×7)","w":14,"h":18,"d":7}];
export default function Page() {
    const [sizeKey,setSizeKey]=useState(sizes[1]?.k||sizes[0].k);
    const [qty,setQty]=useState(1);
    const [activeFaq,setActiveFaq]=useState<number|null>(null);
    const faqItems=[{"q":"How much fabric for a backpack?","a":"Small daypack: 1 yard. Standard: 1.5 yards. Full size: 2 yards. Add 1/2 yard for pockets, flap, and straps."}];
    ...
    return (<div className="container"><Breadcrumb items={[{label:"Bags",href:"/bag"},{label:"Backpack Panel"}]}/>
        <div className="calculator-layout"><div className="calculator-main">
        <div className={styles.toolHeader}><span className="category-badge"><ShoppingBag size={14}/> Bag #374</span><h1>Backpack Panel Calculator</h1><p>Calculate fabric panels for backpack construction.</p></div>
        ...
        <div className="toolbar" style={{marginBottom:10}}>
            <button className="btn btn-secondary btn-sm" onClick={()=>navigator.clipboard.writeText(`${qty} Backpack: ${toFrac(calc.yd)} yd`)}><ClipboardCopy size={13}/> Copy</button>
