...
const sizes = [{"k":"small","n":"Small (5×7)","w":5,"h":7},{"k":"med","n":"Medium (8×10)","w":8,"h":10},{"k":"large","n":"Large (12×15)","w":12,"h":15},{"k":"wine","n":"Wine Bottle (5×14)","w":5,"h":14}];
export default function Page() {
    const [sizeKey,setSizeKey]=useState(sizes[1]?.k||sizes[0].k);
    const [qty,setQty]=useState(1);
    const [activeFaq,setActiveFaq]=useState<number|null>(null);
    const faqItems=[{"q":"How to make gift bags?","a":"Rectangle folded, seamed on sides and bottom, top hemmed with ribbon channel. Add 3 inches to height for drawstring casing."}];
    ...
    return (<div className="container"><Breadcrumb items={[{label:"Bags",href:"/bag"},{label:"Gift Bag"}]}/>
        <div className="calculator-layout"><div className="calculator-main">
        <div className={styles.toolHeader}><span className="category-badge"><ShoppingBag size={14}/> Bag #377</span><h1>Gift Bag Calculator</h1><p>Fabric for reusable gift bags by size.</p></div>
        ...
        <div className="toolbar" style={{marginBottom:10}}>
            <button className="btn btn-secondary btn-sm" onClick={()=>navigator.clipboard.writeText(`${qty} Gift: ${toFrac(calc.yd)} yd`)}><ClipboardCopy size={13}/> Copy</button>
