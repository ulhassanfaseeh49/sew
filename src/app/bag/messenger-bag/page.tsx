...
const sizes = [{"k":"small","n":"Small (10×8×3)","w":10,"h":8,"d":3},{"k":"med","n":"Standard (14×11×4)","w":14,"h":11,"d":4},{"k":"large","n":"Laptop (16×12×5)","w":16,"h":12,"d":5}];
export default function Page() {
    const [sizeKey,setSizeKey]=useState(sizes[1]?.k||sizes[0].k);
    const [qty,setQty]=useState(1);
    const [activeFaq,setActiveFaq]=useState<number|null>(null);
    const faqItems=[{"q":"Features of a messenger bag?","a":"Front flap, adjustable crossbody strap (44-52\"), interior divider, back slip pocket. Use canvas or waxed cotton for durability."}];
    ...
    return (<div className="container"><Breadcrumb items={[{label:"Bags",href:"/bag"},{label:"Messenger Bag"}]}/>
        <div className="calculator-layout"><div className="calculator-main">
        <div className={styles.toolHeader}><span className="category-badge"><ShoppingBag size={14}/> Bag #379</span><h1>Messenger Bag Calculator</h1><p>Fabric for messenger and crossbody bags.</p></div>
        ...
        <div className="toolbar" style={{marginBottom:10}}>
            <button className="btn btn-secondary btn-sm" onClick={()=>navigator.clipboard.writeText(`${qty} Messenger: ${toFrac(calc.yd)} yd`)}><ClipboardCopy size={13}/> Copy</button>
