...
const sizes = [{"k":"card","n":"Card Holder (4×3)","w":4,"h":3},{"k":"bi","n":"Bifold (7×4)","w":7,"h":4},{"k":"tri","n":"Trifold (8×4)","w":8,"h":4},{"k":"clutch","n":"Clutch Wallet (8×5)","w":8,"h":5}];
export default function Page() {
    const [sizeKey,setSizeKey]=useState(sizes[1]?.k||sizes[0].k);
    const [qty,setQty]=useState(1);
    const [activeFaq,setActiveFaq]=useState<number|null>(null);
    const faqItems=[{"q":"Best fabric for wallets?","a":"Cork fabric, vinyl, or heavy cotton with fusible fleece interfacing. Use heavyweight interfacing for structure."}];
    ...
    return (<div className="container"><Breadcrumb items={[{label:"Bags",href:"/bag"},{label:"Wallet Pattern Size"}]}/>
        <div className="calculator-layout"><div className="calculator-main">
        <div className={styles.toolHeader}><span className="category-badge"><ShoppingBag size={14}/> Bag #373</span><h1>Wallet Pattern Size Calculator</h1><p>Calculate fabric for fabric wallets.</p></div>
        ...
        <div className="toolbar" style={{marginBottom:10}}>
            <button className="btn btn-secondary btn-sm" onClick={()=>navigator.clipboard.writeText(`${qty} Wallet: ${toFrac(calc.yd)} yd`)}><ClipboardCopy size={13}/> Copy</button>
