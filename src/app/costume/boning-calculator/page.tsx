...
const sizes = [{"k":"under","n":"Underbust (8 bones)","ct":8,"avg":9},{"k":"over","n":"Overbust (12 bones)","ct":12,"avg":11},{"k":"long","n":"Longline (14 bones)","ct":14,"avg":13}];

export default function Page() {
    const [sizeKey, setSizeKey] = useState(sizes[0].k);
    const [qty, setQty] = useState(1);
    const [activeFaq, setActiveFaq] = useState<number | null>(null);
    const faqItems = [{"q":"Steel vs plastic boning?","a":"Steel spiral: flexible, shapes to body, best for real corsets. Plastic/Rigilene: lighter, for fashion bodices. Steel flat: extra support at center front/back."}];
    const spec = sizes.find((s: typeof sizes[0]) => s.k === sizeKey) || sizes[0];
    const totalLen=spec.ct*spec.avg;const yd=totalLen;
    return (<div className="container"><Breadcrumb items={[{ label: "Costume", href: "/costume" }, { label: "Corset Boning Calculator" }]} />
        <div className="calculator-layout"><div className="calculator-main">
            <div className={styles.toolHeader}><span className="category-badge"><Wand2 size={14} /> Costume #335</span><h1>Corset Boning Calculator</h1><p>Calculate boning length and channels.</p></div>
            ...
            <div className={`glass-card ${styles.calculatorCard}`} style={{ borderLeft: "4px solid hsl(280,50%,45%)" }}>
                ...
                <div style={{ padding: 14, background: "hsl(280,15%,95%)", borderRadius: 10, textAlign: "center" }}>
                    ...
                    <div style={{fontSize:36,fontWeight:800,color:"hsl(280,50%,30%)"}}>{spec.ct}</div><div style={{fontSize:10}}>bones, {totalLen}" total</div>
                </div>
            </div>
