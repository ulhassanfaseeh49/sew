...
const sizes = [{"k":"mini","n":"Mini (16\")","len":16},{"k":"knee","n":"Knee (22\")","len":22},{"k":"midi","n":"Midi (28\")","len":28},{"k":"floor","n":"Floor (40\")","len":40}];

export default function Page() {
    const [sizeKey, setSizeKey] = useState(sizes[0].k);
    const [qty, setQty] = useState(1);
    const [activeFaq, setActiveFaq] = useState<number | null>(null);
    const faqItems = [{"q":"How many layers for a petticoat?","a":"Light poof: 2 layers. Standard costume: 3 layers. Maximum volume: 4-6 layers. Each layer needs its own yardage calculation."}];
    const spec = sizes.find((s: typeof sizes[0]) => s.k === sizeKey) || sizes[0];
    const yd=Math.ceil((spec.len*2+10)/36);
    return (<div className="container"><Breadcrumb items={[{ label: "Costume", href: "/costume" }, { label: "Petticoat Underskirt Yardage" }]} />
        <div className="calculator-layout"><div className="calculator-main">
            <div className={styles.toolHeader}><span className="category-badge"><Wand2 size={14} /> Costume #337</span><h1>Petticoat Underskirt Yardage</h1><p>Calculate yardage for petticoats.</p></div>
