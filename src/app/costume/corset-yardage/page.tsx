...
const sizes = [{"k":"under","n":"Underbust","yd":1},{"k":"over","n":"Overbust","yd":1.5},{"k":"long","n":"Longline","yd":2},{"k":"waist","n":"Waist Cincher","yd":0.75}];

export default function Page() {
    const [sizeKey, setSizeKey] = useState(sizes[0].k);
    const [qty, setQty] = useState(1);
    const [activeFaq, setActiveFaq] = useState<number | null>(null);
    const faqItems = [{"q":"How much fabric for a corset?","a":"Underbust: 1 yard outer + 1 yard lining. Overbust: 1.5 yards each. You need 3 layers minimum: fashion fabric, interlining, and lining."}];
    const spec = sizes.find((s: typeof sizes[0]) => s.k === sizeKey) || sizes[0];
    const yd=spec.yd*qty;
    return (<div className="container"><Breadcrumb items={[{ label: "Costume", href: "/costume" }, { label: "Corset Fabric Yardage Calculator" }]} />
        <div className="calculator-layout"><div className="calculator-main">
            <div className={styles.toolHeader}><span className="category-badge"><Wand2 size={14} /> Costume #334</span><h1>Corset Fabric Yardage Calculator</h1><p>Calculate fabric for corset construction.</p></div>
