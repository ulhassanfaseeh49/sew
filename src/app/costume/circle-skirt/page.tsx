...
const sizes = [{"k":"quarter","n":"Quarter Circle","mult":0.25},{"k":"half","n":"Half Circle","mult":0.5},{"k":"3q","n":"3/4 Circle","mult":0.75},{"k":"full","n":"Full Circle","mult":1},{"k":"double","n":"Double Circle","mult":2}];

export default function Page() {
    const [sizeKey, setSizeKey] = useState(sizes[0].k);
    const [qty, setQty] = useState(1);
    const [activeFaq, setActiveFaq] = useState<number | null>(null);
    const faqItems = [{"q":"Full circle vs half circle skirt?","a":"Full circle: maximum twirl, uses more fabric. Half circle: good drape with less fabric. For costumes: full or double circle for dramatic effect."}];
    const spec = sizes.find((s: typeof sizes[0]) => s.k === sizeKey) || sizes[0];
    const waist=28;const r=waist/(2*Math.PI);const len=22;const needed=Math.ceil((r+len)*2/36*spec.mult*2);const yd=Math.max(needed,2);
    return (<div className="container"><Breadcrumb items={[{ label: "Costume", href: "/costume" }, { label: "Circle Skirt Calculator (Costume)" }]} />
        <div className="calculator-layout"><div className="calculator-main">
            <div className={styles.toolHeader}><span className="category-badge"><Wand2 size={14} /> Costume #338</span><h1>Circle Skirt Calculator (Costume)</h1><p>Calculate fabric for costume circle skirts.</p></div>
            ...
            <div className={`glass-card ${styles.calculatorCard}`} style={{ borderLeft: "4px solid hsl(280,50%,45%)" }}>
                ...
                <div style={{ padding: 14, background: "hsl(280,15%,95%)", borderRadius: 10, textAlign: "center" }}>
                    ...
                    <div style={{fontSize:36,fontWeight:800,color:"hsl(280,50%,30%)"}}>{yd}</div><div style={{fontSize:10}}>yards</div>
                </div>
            </div>
