...
export default function Page() {
  ...
  const qw=parseFloat(quiltW)||0;const qh=parseFloat(quiltH)||0;const oh=parseFloat(overhang)||4;const backW=qw+oh*2;const backH=qh+oh*2;const widthsNeeded=Math.ceil(backW/fw);const totalLen=widthsNeeded*backH;const resultYards=Math.ceil((totalLen/36)*8)/8;const resultLabel=backW.toFixed(0)+"x"+backH.toFixed(0)+" backing";const hasResult=qw>0&&qh>0;

  const faqItems = [{q:"How much overhang for quilt backing?",a:"Standard: 3-4 inches per side. Longarm quilters may need 6-8 inches."}];

  return (
    <div className="container">
      <Breadcrumb items={[{label:"Yardage Calculators",href:"/yardage"},{label:"Quilt Backing Yardage Calculator"}]} />
      <div className="calculator-layout">
        <div className="calculator-main">
          <div className={styles.toolHeader}>
            <span className="category-badge"><span>🟫</span> Yardage Tool #52</span>
            <h1>Quilt Backing Yardage Calculator</h1>
            <p>Backing fabric with overhang for any quilt size.</p>
          </div>
          <div className={`glass-card ${styles.calculatorCard}`}>
            <h2 className={styles.calcTitle}>Enter Dimensions</h2>
            <div className="calculator-form">
              <div className="calculator-form-row"><div className="input-group"><label className="input-label" htmlFor="qw">Quilt width (in)</label><input id="qw" type="number" className="input-field" placeholder="e.g., 60" value={quiltW} onChange={e=>setQuiltW(e.target.value)} min="0"/></div><div className="input-group"><label className="input-label" htmlFor="qh">Quilt height (in)</label><input id="qh" type="number" className="input-field" placeholder="e.g., 80" value={quiltH} onChange={e=>setQuiltH(e.target.value)} min="0"/></div></div><div className="input-group"><label className="input-label" htmlFor="oh">Overhang each side (in)</label><input id="oh" type="number" className="input-field" value={overhang} onChange={e=>setOverhang(e.target.value)} min="0"/></div>
            ...
            {hasResult && (
              <div className={`calculator-results ${styles.results}`}>
                ...
                <div className={styles.resultDetails}>
                  ...
                  <div className={styles.resultRow}><span>Backing size</span><strong>{backW.toFixed(0)}&quot; x {backH.toFixed(0)}&quot;</strong></div><div className={styles.resultRow}><span>Widths of fabric</span><strong>{widthsNeeded}</strong></div>
                </div>
