...
export default function BufferCalcPage() {
    ...
    return (<div className="container"><Breadcrumb items={[{ label: "Yardage Calculators", href: "/yardage" }, { label: "Buffer Calculator" }]} />
        <div className="calculator-layout"><div className="calculator-main">
            <div className={styles.toolHeader}><span className="category-badge"><span>🛡️</span> Yardage Tool #30</span><h1>Extra Yardage Buffer Calculator</h1><p>Calculate exactly how much extra fabric to buy based on your skill, fabric type, and project complexity.</p></div>
            <div className={`glass-card ${styles.calculatorCard}`}>
                ...
                {base > 0 && (<div className={`calculator-results ${styles.results}`}>
                    ...
                    <div className="toolbar"><button className="btn btn-secondary btn-sm" onClick={() => navigator.clipboard.writeText(`Buy ${rounded.toFixed(3)} yd (${base} base + ${totalPct}% buffer)`)}>📋 Copy</button><button className="btn btn-secondary btn-sm" onClick={() => window.print()}>🖨️ Print</button></div>
                </div>)}
            </div>
            <section className="faq-section"><h2>FAQ</h2><div style={{ marginTop: "1.5rem" }}>{faqItems.map((f, i) => (<div key={i} className={`faq-item ${activeFaq === i ? "active" : ""}`}><button className="faq-question" onClick={() => setActiveFaq(activeFaq === i ? null : i)}>{f.q}<svg className="faq-chevron" width="16" height="10" viewBox="0 0 16 10" fill="none"><path d="M1 1L8 8L15 1" stroke="currentColor" strokeWidth="2" strokeLinecap="round" /></svg></button><div className="faq-answer">{f.a}</div></div>))}</div></section>
        </div><aside className="calculator-sidebar"><div className="glass-card related-tools"><h4>Related Tools</h4><a href="/yardage/waste-calculator" className="related-tool-link">♻️ Waste Calculator</a><a href="/yardage/basic-calculator" className="related-tool-link">📐 Basic Yardage</a></div></aside></div>
    </div>);
}
