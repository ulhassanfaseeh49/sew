...
const sizes = [{k:"cocktail",n:"Cocktail (10\")",sz:10},{k:"lunch",n:"Lunch (14\")",sz:14},{k:"dinner",n:"Dinner (18\")",sz:18},{k:"formal",n:"Formal (22\")",sz:22}];

export default function Page() {
    const [sizeKey, setSizeKey] = useState(sizes[0].k);
    const [qty, setQty] = useState(1);
    const [activeFaq, setActiveFaq] = useState<number | null>(null);
    const faqItems = [{"q":"Best fabric for napkins?","a":"Linen: classic. Cotton: affordable. Pre-wash. Finish with narrow hem or mitered corners."}];
    const spec = sizes.find((s: typeof sizes[0]) => s.k === sizeKey) || sizes[0];
    const cut=spec.sz+1;const perWidth=Math.floor(44/cut);const qty2=qty||4;const rows=Math.ceil(qty2/perWidth);const yd=Math.ceil(rows*cut/36*4)/4;
    const resultText = `${qty2} napkins: ${yd} yd`;
    return (<div className="container"><Breadcrumb items={[{ label: "Home Decor", href: "/home-decor" }, { label: "Napkin Calculator" }]} />
        <div className="calculator-layout"><div className="calculator-main">
            <div className={styles.toolHeader}><span className="category-badge"><Home size={14} /> Home #432</span><h1>Napkin Calculator</h1><p>Calculate fabric for cloth napkins.</p></div>
