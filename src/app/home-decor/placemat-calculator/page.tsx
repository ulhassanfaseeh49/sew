...
const sizes = [{k:"std",n:"Standard (18x12)",w:18,h:12},{k:"round",n:"Round (15\" dia)",w:15,h:15},{k:"xl",n:"Large (20x14)",w:20,h:14}];

export default function Page() {
    const [sizeKey, setSizeKey] = useState(sizes[0].k);
    const [qty, setQty] = useState(1);
    const [activeFaq, setActiveFaq] = useState<number | null>(null);
    const faqItems = [{"q":"How to make quilted placemats?","a":"Cut top, batting, and backing same size. Quilt together, bind edges with bias binding."}];
    const spec = sizes.find((s: typeof sizes[0]) => s.k === sizeKey) || sizes[0];
    const cut=spec.w+1;const cutH=spec.h+1;const perWidth=Math.floor(44/cut);const qty2=qty||4;const rows=Math.ceil(qty2/perWidth);const yd=Math.ceil(rows*cutH/36*4)/4;
    const resultText = `${qty2} placemats: ${yd} yd each (top+back)`;
    return (<div className="container"><Breadcrumb items={[{ label: "Home Decor", href: "/home-decor" }, { label: "Placemat Calculator" }]} />
        <div className="calculator-layout"><div className="calculator-main">
            <div className={styles.toolHeader}><span className="category-badge"><Home size={14} /> Home #433</span><h1>Placemat Calculator</h1><p>Calculate fabric for quilted or reversible placemats.</p></div>
