...
const sizes = [{k:"cube",n:"Cube (18\")",sz:18},{k:"round",n:"Round (24\")",sz:24},{k:"rect",n:"Rectangle (36\")",sz:36}];

export default function Page() {
    const [sizeKey, setSizeKey] = useState(sizes[0].k);
    const [qty, setQty] = useState(1);
    const [activeFaq, setActiveFaq] = useState<number | null>(null);
    const faqItems = [{"q":"How to make a cube ottoman?","a":"Cut 6 squares +1 inch. Sew 4 sides + top. Leave bottom open. Stuff with high-density foam."}];
    const spec = sizes.find((s: typeof sizes[0]) => s.k === sizeKey) || sizes[0];
    const yd=spec.sz<20?1.5:spec.sz<30?2:3;
    const resultText = `${yd} yards of upholstery fabric`;
    return (<div className="container"><Breadcrumb items={[{ label: "Home Decor", href: "/home-decor" }, { label: "Ottoman Upholstery Calculator" }]} />
        <div className="calculator-layout"><div className="calculator-main">
            <div className={styles.toolHeader}><span className="category-badge"><Home size={14} /> Home #452</span><h1>Ottoman Upholstery Calculator</h1><p>Fabric for ottoman covering.</p></div>
