...
const sizes = [{k:"love",n:"Loveseat",yd:10},{k:"3seat",n:"3-Seat Sofa",yd:14},{k:"sect",n:"Sectional (per piece)",yd:12},{k:"slip",n:"Slipcover",yd:16}];

export default function Page() {
    const [sizeKey, setSizeKey] = useState(sizes[0].k);
    const [qty, setQty] = useState(1);
    const [activeFaq, setActiveFaq] = useState<number | null>(null);
    const faqItems = [{"q":"Can I do it myself?","a":"Cushion covers: beginner-friendly. Full reupholstery: advanced. Slipcovers: intermediate."}];
    const spec = sizes.find((s: typeof sizes[0]) => s.k === sizeKey) || sizes[0];
    const total=spec.yd;
    const resultText = `${total} yards of 54" upholstery fabric`;
    return (<div className="container"><Breadcrumb items={[{ label: "Home Decor", href: "/home-decor" }, { label: "Sofa Upholstery Calculator" }]} />
        <div className="calculator-layout"><div className="calculator-main">
            <div className={styles.toolHeader}><span className="category-badge"><Home size={14} /> Home #449</span><h1>Sofa Upholstery Calculator</h1><p>Fabric for sofa reupholstery.</p></div>
