...
const sizes = [{k:"16",n:"16\" Square",sz:16},{k:"18",n:"18\" Square",sz:18},{k:"20",n:"20\" Square",sz:20},{k:"24",n:"24\" Square",sz:24}];

export default function Page() {
    const [sizeKey, setSizeKey] = useState(sizes[0].k);
    const [qty, setQty] = useState(1);
    const [activeFaq, setActiveFaq] = useState<number | null>(null);
    const faqItems = [{"q":"Envelope vs zipper?","a":"Envelope: no zipper needed, beginner-friendly. Zipper: neater finish, easier insert removal."}];
    const spec = sizes.find((s: typeof sizes[0]) => s.k === sizeKey) || sizes[0];
    const front=spec.sz+1;const backH=Math.ceil(spec.sz*0.65)+1;const yd=Math.ceil(front*(front+backH*2)/(44*36)*4)/4;
    const resultText = `Front: ${front}", 2 back panels: ${front}" x ${backH}" (${yd} yd)`;
    return (<div className="container"><Breadcrumb items={[{ label: "Home Decor", href: "/home-decor" }, { label: "Envelope Cushion Cover Calculator" }]} />
        <div className="calculator-layout"><div className="calculator-main">
            <div className={styles.toolHeader}><span className="category-badge"><Home size={14} /> Home #435</span><h1>Envelope Cushion Cover Calculator</h1><p>Calculate fabric for envelope-back cushion covers.</p></div>
