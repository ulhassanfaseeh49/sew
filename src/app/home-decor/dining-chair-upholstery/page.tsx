...
const sizes = [{k:"seat",n:"Seat Only",yd:0.33},{k:"both",n:"Seat + Back",yd:0.75},{k:"slip",n:"Slipcover",yd:1.5}];

export default function Page() {
    const [sizeKey, setSizeKey] = useState(sizes[0].k);
    const [qty, setQty] = useState(1);
    const [activeFaq, setActiveFaq] = useState<number | null>(null);
    const faqItems = [{"q":"How to recover dining chairs?","a":"Remove seat (4 screws). Cut fabric 4 inches larger. Stretch and staple underneath."}];
    const spec = sizes.find((s: typeof sizes[0]) => s.k === sizeKey) || sizes[0];
    const total=Math.ceil(spec.yd*qty*4)/4;
    const resultText = `${total} yd for ${qty} chairs`;
    return (<div className="container"><Breadcrumb items={[{ label: "Home Decor", href: "/home-decor" }, { label: "Dining Chair Upholstery Calculator" }]} />
        <div className="calculator-layout"><div className="calculator-main">
            <div className={styles.toolHeader}><span className="category-badge"><Home size={14} /> Home #450</span><h1>Dining Chair Upholstery Calculator</h1><p>Fabric for dining chair seats.</p></div>
