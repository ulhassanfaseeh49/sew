...
const sizes = [{k:"twin",n:"Twin (68x90)",w:68,h:90},{k:"full",n:"Full (86x90)",w:86,h:90},{k:"queen",n:"Queen (90x94)",w:90,h:94},{k:"king",n:"King (104x94)",w:104,h:94}];

export default function Page() {
    const [sizeKey, setSizeKey] = useState(sizes[0].k);
    const [qty, setQty] = useState(1);
    const [activeFaq, setActiveFaq] = useState<number | null>(null);
    const faqItems = [{"q":"How much fabric?","a":"Twin: 6 yd. Queen: 10-12 yd. King: 12-14 yd. Use 90-108 inch sheeting to avoid seams."}];
    const spec = sizes.find((s: typeof sizes[0]) => s.k === sizeKey) || sizes[0];
    const cutW=spec.w+1;const cutH=spec.h+1;const panels=Math.ceil(cutW/44);const yd=Math.ceil(panels*cutH/36*2*4)/4;
    const resultText = `${cutW}" x ${cutH}" (${panels} panels x2 = ${yd} yd)`;
    return (<div className="container"><Breadcrumb items={[{ label: "Home Decor", href: "/home-decor" }, { label: "Duvet Cover Calculator" }]} />
        <div className="calculator-layout"><div className="calculator-main">
            <div className={styles.toolHeader}><span className="category-badge"><Home size={14} /> Home #442</span><h1>Duvet Cover Calculator</h1><p>Calculate fabric for duvet covers.</p></div>
