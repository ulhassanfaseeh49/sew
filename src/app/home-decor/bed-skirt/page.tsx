...
const sizes = [{k:"twin",n:"Twin",w:39,h:75,drop:14},{k:"full",n:"Full",w:54,h:75,drop:14},{k:"queen",n:"Queen",w:60,h:80,drop:14},{k:"king",n:"King",w:76,h:80,drop:14}];

export default function Page() {
    const [sizeKey, setSizeKey] = useState(sizes[0].k);
    const [qty, setQty] = useState(1);
    const [activeFaq, setActiveFaq] = useState<number | null>(null);
    const faqItems = [{"q":"Standard drop?","a":"14-15 inches. Measure from box spring top to floor. Low-profile: 10-11 inches."}];
    const spec = sizes.find((s: typeof sizes[0]) => s.k === sizeKey) || sizes[0];
    const perim=(spec.w+spec.h)*2;const ratio=2.5;const ungathered=Math.ceil(perim*ratio);const yd=Math.ceil(ungathered*(spec.drop+2)/(44*36)*4)/4;
    const resultText = `${ungathered}" ungathered, ${yd} yd`;
    return (<div className="container"><Breadcrumb items={[{ label: "Home Decor", href: "/home-decor" }, { label: "Bed Skirt Calculator" }]} />
        <div className="calculator-layout"><div className="calculator-main">
            <div className={styles.toolHeader}><span className="category-badge"><Home size={14} /> Home #445</span><h1>Bed Skirt Calculator</h1><p>Calculate fabric for bed skirts.</p></div>
