...
const sizes = [{k:"4",n:"4-seat (36\")",len:36},{k:"6",n:"6-seat (54\")",len:54},{k:"8",n:"8-seat (72\")",len:72},{k:"10",n:"10-seat (90\")",len:90}];

export default function Page() {
    const [sizeKey, setSizeKey] = useState(sizes[0].k);
    const [qty, setQty] = useState(1);
    const [activeFaq, setActiveFaq] = useState<number | null>(null);
    const faqItems = [{"q":"Standard table runner width?","a":"12-14 inches. Width should be about 1/3 of table width."}];
    const spec = sizes.find((s: typeof sizes[0]) => s.k === sizeKey) || sizes[0];
    const width=14;const overhang=6;const cutL=spec.len+overhang*2+1;const cutW=width+1;const yd=Math.ceil(cutL/36*4)/4;
    const resultText = `${cutW}" x ${cutL}" (${yd} yd)`;
    return (<div className="container"><Breadcrumb items={[{ label: "Home Decor", href: "/home-decor" }, { label: "Table Runner Calculator" }]} />
        <div className="calculator-layout"><div className="calculator-main">
            <div className={styles.toolHeader}><span className="category-badge"><Home size={14} /> Home #430</span><h1>Table Runner Calculator</h1><p>Calculate fabric for table runners.</p></div>
