...
const sizes = [{k:"narrow",n:"Narrow (1/8\" cord)",cord:0.125},{k:"med",n:"Medium (3/16\")",cord:0.1875},{k:"thick",n:"Thick (1/4\")",cord:0.25},{k:"jumbo",n:"Jumbo (1/2\")",cord:0.5}];

export default function Page() {
    const [sizeKey, setSizeKey] = useState(sizes[0].k);
    const [qty, setQty] = useState(1);
    const [activeFaq, setActiveFaq] = useState<number | null>(null);
    const faqItems = [{"q":"How wide for piping strips?","a":"Cord circumference + 2x seam allowance. Always cut on bias for curves."}];
    const spec = sizes.find((s: typeof sizes[0]) => s.k === sizeKey) || sizes[0];
    const perim=80;const sa=0.5;const stripW=Math.round((spec.cord*Math.PI+sa*2)*8)/8;const yd=Math.ceil(perim*stripW/(44*36)*4)/4;
    const resultText = `${perim}" piping, ${stripW}" wide strips (${yd} yd)`;
    return (<div className="container"><Breadcrumb items={[{ label: "Home Decor", href: "/home-decor" }, { label: "Piping/Welting Calculator" }]} />
        <div className="calculator-layout"><div className="calculator-main">
            <div className={styles.toolHeader}><span className="category-badge"><Home size={14} /> Home #446</span><h1>Piping/Welting Calculator</h1><p>Calculate bias strips for piping.</p></div>
