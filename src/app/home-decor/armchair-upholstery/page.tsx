...
const sizes = [{k:"dining",n:"Dining (seat only)",yd:0.5},{k:"accent",n:"Accent Chair",yd:4},{k:"club",n:"Club Chair",yd:5},{k:"wing",n:"Wingback",yd:7}];

export default function Page() {
    const [sizeKey, setSizeKey] = useState(sizes[0].k);
    const [qty, setQty] = useState(1);
    const [activeFaq, setActiveFaq] = useState<number | null>(null);
    const faqItems = [{"q":"How much fabric for reupholstery?","a":"Dining seat: 1/2 yd. Accent: 4-5 yd. Club: 5-6 yd. Wingback: 6-8 yd."}];
    const spec = sizes.find((s: typeof sizes[0]) => s.k === sizeKey) || sizes[0];
    const welt=spec.yd>3?1:0;const total=spec.yd+welt;
    const resultText = `${total} yd (${spec.yd} fabric + ${welt} welting)`;
    return (<div className="container"><Breadcrumb items={[{ label: "Home Decor", href: "/home-decor" }, { label: "Armchair Upholstery Calculator" }]} />
        <div className="calculator-layout"><div className="calculator-main">
            <div className={styles.toolHeader}><span className="category-badge"><Home size={14} /> Home #448</span><h1>Armchair Upholstery Calculator</h1><p>Fabric for armchair reupholstery.</p></div>
